...
def readData(fileName):
    tokens = [] #All words in txt file
    tags = [] #All tags in txt file
...
def createUnigrams(tokens, tags):
    tokenTags = {} #Tokens and all their corresponding tags from the txt file
    uniqueTags = set(tags) #Unique tags
...
def mostProbablePOS(dictionary):
    ...
    for key, value in dictionary.items():
        ...
        dictionary[key] = maxValue[0]  # All possible tags for the word are replaced by the most probable tag
...
def mostProbableErrors(tokens, tags, dictionary):
    ...
    for word in tokens: 
        modTags.append(dictionary[word])  #Most probable tags of all words in file

    for i in range(len(modTags)): #If most probable tag is not goal tag, error increases
        if modTags[i] != tags[i]:
            error += 1

    #Print most probable tags in a txt file
...
if __name__ == '__main__':
    ...
    #-------------------Brill's tagging---------------------------------------------------
    fileName = 'NLP6320_POSTaggedTrainingSet-Windows(1).txt'
    tokens, tags = readData(fileName)
